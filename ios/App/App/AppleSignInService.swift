...
struct AppleSignInCredential: Codable, Equatable {
    ...
    var displayName: String {
        let parts = [givenName, familyName].compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty }
...
enum AppleSignInError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingIdentityToken:
            return "Не удалось получить данные Apple ID. Попробуйте ещё раз."
        case .cancelled:
            return "Вход через Apple был отменён."
        case .failed:
            return "Не удалось войти через Apple. Попробуйте ещё раз."
        }
    }
}
