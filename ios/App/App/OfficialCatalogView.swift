...
struct OfficialCatalogProduct: Equatable, Identifiable {
    ...
    var discountPercent: Int? { discountPercent(at: Date()) }
...
enum OfficialCatalogPromotionParser {
    ...
    static func expiryDate(from rawValue: Any?, fetchedAt: Date = Date()) -> Date? {
        ...
        // Retailers often omit the year around New Year. Only roll forward when the
        // parsed day would otherwise be more than half a year in the past.
...
enum OfficialCatalogPriceBasisParser {
    static func parse(
        ...
        // Keep package products as pieces unless the official page explicitly states a basis.
        _ = productName
        return (1, .piece)
    }
}

/// Drops scraper garbage: non-product files, UI chrome, absurd prices, food leaks in household.
enum OfficialCatalogProductQuality {
    static let minimumPrice = 1.0
    static let maximumPrice = 50_000.0
    ...
    static func cleanSummary(_ rawSummary: String?, productName: String = "") -> String? {
        ...
        // Remove emojis
        text = text.unicodeScalars.filter { !($0.properties.isEmoji && $0.properties.isEmojiPresentation) }.map(String.init).joined()
    ...
    static func isAcceptableProductURL(_ url: URL) -> Bool {
        ...
        // ATB / Silpo / Zakaz (Auchan, Novus, Metro)
        if productPathPattern.firstMatch(in: path, range: range) != nil {
            return true
        }

        // VARUS uses bare product slugs: /zasib-dlya-mytya-posudy-...
        if host == "varus.ua" || host.hasSuffix(".varus.ua") {
            ...
            return slug.count >= 4
                ...
                && !slug.contains(".")
                && slug.contains(where: { $0.isLetter })
        }
    ...
    static func sanitizedPrices(
        ...
        if let originalPrice,
           ...
           originalPrice <= maximumPrice {
            ...
            // Store promos can be deep (3–6× list price); above ~10× is parse noise.
            cleanOriginal = ratio <= 10.0 ? originalPrice : nil
        } else {
            cleanOriginal = nil
        }

        let cleanLoyalty: Double?
        if let loyaltyPrice,
           loyaltyPrice >= minimumPrice,
           loyaltyPrice < price {
            cleanLoyalty = loyaltyPrice
        } else {
            cleanLoyalty = nil
        }
    ...
    static func upgradedImageURL(_ url: URL?) -> URL? {
        ...
        // Normalize odd absolute forms from store JSON-LD (e.g. https://varus.ua:443/...)
        ...
        let replacements = [
            ...
            // Silpo often serves smaller list thumbs; prefer larger gallery size when path allows.
        ...
        // Drop empty / placeholder data-uri leftovers
    ...
    static func isPlausibleForHouseholdRoute(_ product: OfficialCatalogProduct) -> Bool {
        // Food categories on a household page are scraper / nav leaks, not home goods.
        ...
        let groups: [(ProductCategory, CategoryVocabulary)] = [
            ...
            (
                ...
                CategoryVocabulary(
                    ...
                    prefixes: ["яблуч", "яблоч", "банан", "томат", "помід", "помид", "огір", "огур", "картоп", "картоф", "цибул", "морк", "апельс", "овоч", "овощ", "авокад", "кабач", "баклаж", "капуст", "персик", "виноград", "полуниц", "клубник", "малин", "груш", "ягод", "ягід"],
                    phrases: ["овочі та фрукти", "овощи и фрукты", "свіжі овочі", "свежие овощи", "свіжі фрукти", "свежие фрукты"]
                )
            ),
        ]
