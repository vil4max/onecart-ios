...
final class FamilySpace: NSManagedObject {
    ...
    var needsRemoteCreationValue: Bool { needsRemoteCreation?.boolValue ?? false }
...
final class StoreEntity: NSManagedObject {
    ...
    var stableID: UUID { id ?? UUID() }
    var displayName: String { name?.nilIfBlank ?? "Магазин" }
    var displayIcon: String { icon?.nilIfBlank ?? String(displayName.prefix(2)).uppercased() }
    var displayColorHex: String { colorHex?.nilIfBlank ?? "#34785B" }
    var latitudeValue: Double? { latitude?.doubleValue }
    var longitudeValue: Double? { longitude?.doubleValue }
    var isPinnedValue: Bool { isPinned?.boolValue ?? false }
    var createdDate: Date { createdAt ?? .distantPast }
    var updatedDate: Date { updatedAt ?? createdDate }
    var isDeletedValue: Bool { deletedAt != nil }
...
final class ShoppingListEntity: NSManagedObject {
    ...
    var stableID: UUID { id ?? UUID() }
    var displayTitle: String { title?.nilIfBlank ?? "Список покупок" }
...
enum ProductCategory: String, CaseIterable, Identifiable {
    ...
    var localizedName: String {
        ...
        case .household: return "Для дома"
...
final class ProductEntity: NSManagedObject {
    ...
    var noteValue: String { note ?? "" }
    var isPurchasedValue: Bool { isPurchased?.boolValue ?? false }
    var createdDate: Date { createdAt ?? .distantPast }
    var updatedDate: Date { updatedAt ?? createdDate }
    var isDeletedValue: Bool { deletedAt != nil }
}
