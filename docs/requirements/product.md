...
| REQ-AUTH-040 | `AppleSignInTests` → `appleSignInCredentialBuildsDisplayNameAndAccountID`; `ManagedObjectModelTests` → `memberProfileBelongsToTheCartWithOptionalFields`; `MemberProfileTests` → `@Test "a name change publishes one profile into the active cart, and repeats write nothing"`, `@Test "a member publishes the profile into the shared cart's store"`, `@Test "duplicate rows of one member resolve to the newest, and an upsert tombstones the rest"`, `@Test "clearing the name withdraws it from the profile so members fall back"`, `@Test "without an iCloud identity nothing is published"`, `@Test "a store written by the previous model opens with the member profile entity"` |
